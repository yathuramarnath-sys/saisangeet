...
const { getTenantIdForIdentifier } = require("../../data/users-index");
const { runWithTenant } = require("../../data/tenant-context");

// ─── Find a user within the CURRENT tenant context ───────────────────────────
 ...
 * 1. Look up the tenantId in the global index
 * 2. Run the search inside that tenant's context
 */
async function findUserByIdentifier(identifier) {
  // ── 1. Try Postgres ────────────────────────────────────────────────────────
  try {
    const result = await query(
      `SELECT u.id, u.outlet_id AS "outletId", u.full_name AS "fullName",
              u.email, u.phone, u.password_hash AS "passwordHash", u.status,
  ...
  return new Promise((resolve) => {
    runWithTenant(tenantId, () => {
      resolve(findUserInStore(identifier));
    });
  });
}

/**
 * Expose tenant resolution so the login service can embed tenantId in the JWT.
 */
function getTenantForIdentifier(identifier) {
  return getTenantIdForIdentifier(identifier);
}

module.exports = { findUserByIdentifier, getTenantForIdentifier };
