...
export function LiveScreen() {
  ...
  const totalSales   = summary?.totalRevenue   ?? summary?.total   ?? 0;
  const orderCount   = summary?.orderCount     ?? summary?.orders  ?? 0;
  const avgBill      = orderCount > 0 ? totalSales / orderCount : 0;
  const gstCollected = summary?.totalTax       ?? summary?.gst     ?? 0;
  const cashTotal    = summary?.cashTotal      ?? summary?.cash    ?? 0;
  const upiTotal     = summary?.upiTotal       ?? summary?.upi     ?? 0;
  const cardTotal    = summary?.cardTotal      ?? summary?.card    ?? 0;
