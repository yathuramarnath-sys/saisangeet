...
export function MenuPanel({ categories, menuItems, onAddItem }) {
  ...
  const filtered = useMemo(() => {
    ...
    const base = q
      ...
      : menuItems.filter((i) => i.category === activeCategory || i.categoryName === activeCategory);
  ...
  return (
    <div className="menu-panel">
      ...
      <div className="menu-search-wrap">
        <svg className="menu-search-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
        ...
        />
        {search && (
          <button type="button" className="menu-search-clear" onClick={() => setSearch("")}>✕</button>
        )}
      </div>
        ...
        {filtered.map((item) => (
          ...
          >
            ...
            <span className="menu-item-price">
              ₹{typeof item.price === "number" ? item.price : Number(String(item.price || "").replace(/[^\d.]/g, "")) || 0}
            </span>
          </button>
        ))}
      </div>
