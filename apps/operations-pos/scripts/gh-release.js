...
// latest.yml is generated by electron-builder alongside the .exe.
// electron-updater NEEDS this file in the GitHub Release to detect new versions.
const latestYml   = path.resolve(__dirname, "../electron-dist/latest.yml");
...
if (!fs.existsSync(latestYml)) {
  console.error(`[release] ERROR: ${latestYml} not found. electron-builder should have created it.`);
  process.exit(1);
}
...
// IMPORTANT: both the .exe AND latest.yml must be uploaded.
//   - Plato-POS-Setup.exe  → the actual installer cashiers download
//   - latest.yml           → metadata file electron-updater reads to detect new versions
//     Without latest.yml, running POS apps will NEVER see the update banner.
console.log(`[release] Creating GitHub Release ${tag}…`);
execSync(
  `gh release create ${tag} "${exePath}" "${latestYml}" ` +
  `--repo ${owner}/${repo} ` +
