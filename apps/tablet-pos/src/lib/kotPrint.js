/* ══════════════════════════════════════════════════════════════════════════════
   KOT PRINT UTILITY — Android Tablet POS
   Generates a thermal-style KOT and prints it via WiFi network proxy.
   Works for 80mm and 58mm paper widths.
   ══════════════════════════════════════════════════════════════════════════════ */

import { tabletPrintKOT } from "./wifiPrint";
...
<body>
  ...
  // ── Production path: WiFi network print (Android tablet) ──────────────────
  tabletPrintKOT(html, options.station || order.station, paper === "58mm" ? 58 : 80)
    .then((result) => {
      if (!result?.ok && !result?.fallback) {
        window.dispatchEvent(new CustomEvent("dinex:print-error", {
          detail: { source: "KOT", error: result?.error },
        }));
      }
    })
    .catch((err) => {
      window.dispatchEvent(new CustomEvent("dinex:print-error", {
        detail: { source: "KOT", error: err?.message || "Print failed" },
      }));
    });
