/**
 * printBill.js — thermal bill printing for Android tablet POS
 *
 * Production path: WiFi network print via wifiPrint proxy
    ...
    .then((result) => {
      if (!result?.ok && !result?.fallback) {
        window.dispatchEvent(new CustomEvent("dinex:print-error", {
          detail: { source: "Bill", error: result?.error },
        }));
      }
    })
    .catch((err) => {
      window.dispatchEvent(new CustomEvent("dinex:print-error", {
        detail: { source: "Bill", error: err?.message || "Print failed" },
      }));
    });

  return;

  // ── Fallback: system print dialog (handled inside tabletPrintBill) ─────────
