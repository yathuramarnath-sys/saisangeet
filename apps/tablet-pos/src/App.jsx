...
import { setCategoryAvailability } from "./lib/categoryAvailability.js";
...
export function App() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        // ── Tablet: connect TO the Windows POS local server (client mode) ──────
        {
        const savedLocalIp = localStorage.getItem("tablet_local_server_ip")?.trim();
        const localBase    = savedLocalIp ? `http://${savedLocalIp}:4001` : "http://dinexpos-server.local:4001";
        const localSock = io(localBase, {
          reconnection:         true,
        ...
        localSock.on("connect", () => {
          // Tablet joins as a client — receives order state from POS
  ...
  async function handleSettle(paymentsInput) {
    ...
    // Auto-advance to a fresh counter order after settling a takeaway/delivery ticket
    if (order.isCounter) handleNewCounterOrder();
  ...
  function handleHoldToggle() {
    if (!selectedTableId) return;
    const isCounter = orders[selectedTableId]?.isCounter;
    const wasOnHold = orders[selectedTableId]?.isOnHold;
    mutateOrder(selectedTableId, o => {
      o.isOnHold = !o.isOnHold;
      showToast(o.isOnHold ? "Order put on hold" : "Order resumed");
      return o;
    });
    // Auto-advance to a fresh counter order when putting a counter ticket on hold
    if (!wasOnHold && isCounter) {
      handleNewCounterOrder();
    }
  }
  ...
  async function handleSaveCustomer(data) {
    ...
    // Auto-apply loyalty discount if visit-stamp or birthday rule matches
    if (!data.phone) return;
    try {
      const res = await api.get(`/customers/visit-count?phone=${encodeURIComponent(data.phone)}`);
      const visitCount   = res?.visitCount || 0;
      const savedMonth   = data.birthdayMonth || res?.customer?.birthdayMonth || "";
      const currentMonth = new Date().toLocaleString("en-US", { month: "long" });

      // Find matching loyalty rules scoped to this outlet
      const outletName = outlet?.name || "";
      const loyaltyRules = discountRules.filter(r =>
        r.isActive !== false &&
        (r.outletScope === "All Outlets" || !r.outletScope || r.outletScope === outletName) &&
        (r.triggerType === "visit_stamp" || r.triggerType === "birthday")
      );

      let matchedRule = null;

      // Birthday takes priority if current month matches
      if (savedMonth && savedMonth === currentMonth) {
        matchedRule = loyaltyRules.find(r => r.triggerType === "birthday") || null;
      }

      // Visit stamp — check if visitCount+1 (this visit) is a multiple of N
      if (!matchedRule) {
        const nextVisit = visitCount + 1;
        matchedRule = loyaltyRules.find(r =>
          r.triggerType === "visit_stamp" &&
          (r.visitStampEvery || 5) > 0 &&
          nextVisit % (r.visitStampEvery || 5) === 0
        ) || null;
      }

      if (!matchedRule) return;

      // Compute discount amount from the rule
      const tableId = selectedTableId;
      ...
      if (matchedRule.discountType === "percentage") {
        discountAmt = Math.round((subtotal * Number(matchedRule.value)) / 100 * 100) / 100;
      } else {
        discountAmt = Math.min(Number(matchedRule.value), subtotal);
      }

      if (discountAmt <= 0) return;

      mutateOrder(tableId, o => { o.discountAmount = discountAmt; return o; });
      api.patch("/operations/order/discount", { tableId, outletId: outlet?.id, discountAmount: discountAmt }).catch(() => {});

      const label = matchedRule.triggerType === "birthday"
        ? `🎂 Birthday discount applied — ${matchedRule.name}`
        : `🎯 Loyalty discount applied — ${matchedRule.name} (visit #${visitCount + 1})`;
      showToast(label, "success");
    } catch {
      // non-critical — don't block cashier on loyalty check failure
    }
  }
  ...
  function handleCancelKotItem(itemId, reason, qty = 1) {
    if (!selectedTableId) return;
    const tableId = selectedTableId;
    const order   = ordersRef.current[tableId];
    const itemIdx = (order?.items || []).findIndex(i => i.id === itemId);
    if (itemIdx === -1) return;
    const item = order.items[itemIdx];
    if (!item || item.isVoided || !item.sentToKot) return;

    const cancelQty = Math.min(qty, item.quantity);
    const newQty    = Math.round((item.quantity - cancelQty) * 1000) / 1000;
    const willVoid  = newQty <= 0;

    mutateOrder(tableId, o => {
      if (!o.items[itemIdx]) return o;
      if (willVoid) {
        o.items[itemIdx].isVoided   = true;
        o.items[itemIdx].voidReason = reason;
      } else {
        o.items[itemIdx].quantity = newQty;
      }
      return o;
    });
    ...
    showToast(`${cancelQty}× ${item.name} cancelled from kitchen`);
  }
